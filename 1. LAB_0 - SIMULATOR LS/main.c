...
void func_5()
{
	...
	for(int f = 0; f < i; f++)
	{
		...
		if(file_st_1.st_mode & S_IXUSR)
		{
			...
			{
				printf("d%s  %lu  %s  %8s", mode, file_st_1.st_nlink, pwd->pw_name, gro->gr_name);
				printf(" %6ld  %s \x1b[1;34m %-8s \x1b[0m \n", file_st_1.st_size, buf_time, Files[f]);
			}
			else
			{
				printf("-%s  %lu  %s  %8s", mode, file_st_1.st_nlink, pwd->pw_name, gro->gr_name);
