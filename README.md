...
#		3) Пишущий поток записывает в общий массив номер записи (монотонно возрастающий счетчик), 
#			а читающие потоки выводят на экран свой tid и текущее состояние общего массива.
...
# 9. LAB_6 - pthread_cond:
# 1.1 Требования:
#		1) Создать 10 читающих потоков и 1 пишущий.   
#  		2) Потокам должен быть доступен один и тот же массив символов.
#		3) Пишущий поток записывает в общий массив номер записи (монотонно возрастающий счетчик), 
#			а читающие потоки выводят на экран свой tid и текущее состояние общего массива.
#		4) Доступ должен быть синхронизирован с помощью условных переменных.
#		5) Проверить лабораторную работу на кафедральном сервере !!!
#		6) Срок сдачи: 16.12.2021; (Зачтена: В ожидании - 15.11.2021[Вовремя]).
#
# 1.2 Статус: - Зачтена;
#
...
#		3) Пишущий поток записывает в общий массив номер записи (монотонно возрастающий счетчик), 
#		а читающие потоки выводят на экран свой tid и текущее состояние общего массива.
#		4) Доступ должен быть синхронизирован с помощью блокировок чтения-записи(rwlock).
#		5) Проверить лабораторную работу на кафедральном сервере !!!
#		6) Срок сдачи: 23.12.2021; (Зачтена: В ожидании - 15.11.2021[Вовремя]).
#
# 1.2 Статус: -Зачтена;
#